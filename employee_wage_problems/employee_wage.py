...
class Employee:
    ...
    def calculate_monthly_wage(self):
        while self.days < self.no_of_days and self.hours < self.no_of_hours:
            self.days += 1
            self.hours += self.calculate_daily_wage()
...
class Company:
    ...
    def add(self, emp):
        if emp.name not in self.employee_dict:
            self.employee_dict[emp.name] = emp

    def display(self, employee_name):
        if employee_name in self.employee_dict:
            emp = self.employee_dict[employee_name]
            print(f"{emp.name} worked for {emp.days} days {emp.hours} hours; monthly wage is {emp.wage}")
        else:
            print("Employee not found")

    def update(self, employee_name, wage_per_hour):
        if employee_name in self.employee_dict:
            self.employee_dict[employee_name].wage_per_hour = wage_per_hour
            self.employee_dict[employee_name].calculate_monthly_wage()

    def delete(self, employee_name):
        if employee_name in self.employee_dict:
            del self.employee_dict[employee_name]
        else:
            print("Employee not found")
...
def main():
    ...
    while True:
        ...
        if choice == 1:
            ...
            if company_name in MC.companies:
                if employee_name in MC.companies[company_name].employee_dict:
